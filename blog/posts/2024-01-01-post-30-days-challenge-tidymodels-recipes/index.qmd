...
data_eeg <-
  read_csv(
    here(
      "blog/posts/2024-01-01-30-days-challenge-tidymodels-recipes/data_eeg.csv"
    )
  ) |>
  clean_names() |>
  rename(id = x1)
...
data_ga <- read_csv(
  here(
    "blog/posts/2024-01-01-30-days-challenge-tidymodels-recipes/data_google_merch.csv"
  )
)
...
```{r day-26-count-shipping}
ga_tr |>
  count(shipping_tier, sort = TRUE)
```

```{r day-26-vis-shipping}
ggplot(
...
```{r day-26-baked-upsample-over-vis}
ggplot(
...
```{r day-27-recipe-pad-value-mean}
ga_rec <- recipe(~., data = data_ga) |>
...
```{r day-27-rec-month-other-value}
drinks_rec <- recipe(~., data = drinks_tr) |>
...
```{r day-28-rec-simple-many-interactions}
credit_rec <- recipe(~., data = credit_tr) |>
  step_interact(
...
```{r day-28-rec-dummy-interaction}
...
```{r day-29-rec-col-bad-check, error=TRUE}
credit_rec <- recipe(~., data = credit_tr) |>
  check_class(
    Status,
    Home,
...
```{r day-29-rec-check-cols, error = TRUE}
credit_rec <- recipe(~., data = credit_tr) |>
...
```{r day-29-penguins-check-missing, error=TRUE}
...
```{r day-29-rec-fail-missing-new-data}
# No error
penguin_rec <- recipe(~., data = penguins_tr) |>
...
```{r day-29-missing-fail-bake, error=TRUE}
# Error
