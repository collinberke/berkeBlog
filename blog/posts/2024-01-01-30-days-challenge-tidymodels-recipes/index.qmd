...
Let's pivot topics at this point in this post. Another type of preprocessing step `recipes` can perform is data transformations. For today's example, I'll spend some time learning about `step_log()`. `step_log()` creates a recipe step that will log transform data.
...
Why use a log transformation? In most of what I've read, a log transformation for a variable is used to transform skewed distributions to a distribution that is more normal. This normality is important because some models (e.g., linear regression) assume variables are normally distributed. In addition, when employed in modeling contexts, its application may lead to more accurate model predictions. Several of the articles stated log transformations are useful when working in domains that tend to have variables with skewed distributions, like financial data (i.e., salaries, housing prices, etc.).

The answers to the above questions are quite general and simplified, so here are a few resources I found that gave me a deeper understanding of the topic:

* [Logs (logarithims), Clearly Explained!!! - @statquest YouTube video](https://www.youtube.com/watch?v=VSi0Z04fWj0)
* [What is the reason behind taking log transformation of few continuous variables? - Stack Exhchange post](https://datascience.stackexchange.com/questions/40089/what-is-the-reason-behind-taking-log-transformation-of-few-continuous-variables)
