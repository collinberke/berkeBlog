...
server <- function(input, output, session) {
  observeEvent(input$back, {
    message(glue(
      "The user clicked the `Back` button. It's value is now {input$back}"
    ))
  })

  observeEvent(input$forward, {
    message(glue(
      "The user clicked the `Next` button. It's value is now {input$forward}"
    ))
  })

  output$narrative <- renderText(glue(
    "Back is {input$back}. Forward is {input$forward}"
  ))
}
