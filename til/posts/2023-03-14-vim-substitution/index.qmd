...
Today I learned how to find and replace in Vim. I have found knowing a few variations of the `substitute` (`:s` or `su` for short) to be powerful skill to quickly and efficiently edit code and text. By knowing a few simple variations to the command, you can greatly improve your productivity. You just have to know the command's different patterns and when to apply them.

This TIL post aims to highlight some of the basics of using the `:s` command in Vim. It also uses several examples to show some basic applications of the command, as my intention is to get you up and running quickly with using `:s`. Although most of the examples here use the R programming, these concepts can be applied to any programming language or text editing task. 

The substitute command provides a lot of utility and various options to perform find and replace editing tasks, and this post focuses on the basics. If you're looking to learn more davanced features, I suggest reading the docs (`:help substitute`). I'll also provide some additional links to other resources throughout and near the end of the post if you're interested in learning more.
...
`:s` can be used to find each occurance of a text string, and replace it with another text string. Say I have character vector `basket` full of fruit in the below example. However, I don't like oranges in my fruit basket and want apples instead. I can enter the following into the command prompt to find the string `orange` and replace it with the string `apple`:
...
To make this even easier, you can start on your current line `.` and specify to Vim how many additional lines you would like to find and replace in the range. Keep in mind `.` represents the current line your cursor is located currently within the file. The command looks like this:

`:.,+2s/carrot/kale/g`

What if I had a much bigger basket, and I just wanted to start at my current location and replace all instances that follow? We can use the `$` in the range. The use of the dollar sign means to the end of the file.
...
* Find and replace one string or many strings in a file.
* Find and replace based on a specified range.
* Find and replace using objects stored in your search buffer.
* Find and replace using text under your cursor.
